...
public class ActiveUserServiceImpl extends RemoteServiceServlet 
    ...
    static final private String dir = "../../../data/crowdlogs/";
    ...
    private void runLogParser(int window) throws IOException, ParseException {
        ...
        Map<String, Collection<Long>> userDays = CrowdLogParser.parseAuthenticationEvents(new File(dir), startDate.getTime(), endDate.getTime());
    ...
    private Collection<Row> createRows(Collection<String> names, Map<String, Collection<Long>> userDays) {
        ...
        for (String name : names) {
            ...
            Row row = new Row(name, df.format(days.iterator().next()));
            rows.add(row);
        }
        return rows;
    }
